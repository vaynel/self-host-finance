...
import { Search, Filter, Plus, ArrowUpRight, ArrowDownRight, ArrowLeftRight } from "lucide-react";
            ...
            <div key={tx.id} className="px-5 py-3.5 flex items-center justify-between hover:bg-muted/30 transition-colors cursor-pointer">
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  tx.type === "income" && "bg-accent",
                  tx.type === "expense" && "bg-destructive/10",
                  tx.type === "transfer" && "bg-secondary",
                )}>
                  {tx.type === "income" && <ArrowUpRight className="h-4 w-4 text-income" />}
                  {tx.type === "expense" && <ArrowDownRight className="h-4 w-4 text-expense" />}
                  {tx.type === "transfer" && <ArrowLeftRight className="h-4 w-4 text-muted-foreground" />}
                </div>
                <div>
